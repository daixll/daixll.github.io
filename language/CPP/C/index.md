...
4. **静态数组：** 如果你在程序中定义一个静态数组，并且它的大小在编译时已知，那么该数组会被存储在数据段中。例如：

   ```c
   int staticArray[100];
   ```

   在这个例子中，`staticArray`是一个具有100个整数元素的静态数组，它会被存储在数据段中。

这些数据在程序运行期间保持不变，它们的内存空间在程序加载时就被分配好，并且在整个程序执行期间保持不变。
