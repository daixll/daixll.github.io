...
> 设 $X_1, X_2, \cdots, X_6$ 来自总体 $N(0,1)$，$Y = (X_1+X_2+X_3)^2 + (X_4+X_5+X_6)^2$，试确定常数 $C$，使 $CY$ 服从 $\chi^2$ 分布。

* $N(0,1)$，说明 $X_1, X_2, \cdots, X_6$ 服从标准正态分布（均值0，方差1）。

* 结合 $\chi^2$ 分布的 **定义**，观察 $Y$ 的形式。

$Y$ 为两个标准正态分布的平方和，即 $Y \sim \chi^2(2)$。
